...
public final class TreeValueExpression extends org.activiti.engine.impl.javax.el.ValueExpression {
   ...
   * same tree structure will also have the same structural id and vice versa.
	...
	public String getIdentifier(Node nodes){
		if(nodes==null){
			nodes=node;
		} 
		System.out.println("TreeValueExpression.getIdentifier()"); 
		for (int i = 0; i <nodes.getCardinality(); i++) {
			Node n =nodes.getChild(i);
//			if(n instanceof AstIdentifier){ 
				System.out.println("nodes.getChild(i).toString()="
						+ n.toString());
//			}else{
				getIdentifier(n); 
//			}
