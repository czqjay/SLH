...
public class ConcurrentSessionFilter extends GenericFilterBean {
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
        ...
        if (session != null) {
        	logger.debug("ConcurrentSessionFilter session:"+session.getId());
            ...
            if (info != null) { 
            	...
            	if(session.getAttribute("userAccount")!=null){   
                 ...
                 csl.setAccount(session.getAttribute("userAccount").toString());    
                 List<ClusterSingleLogin> list =cslmpl.findByExample(csl);   
                 if(!list.isEmpty()){
                 	csl =list.get(0); 
                 	if(!StringUtils.isBlank(csl.getSessionId()))
	                 	if(!csl.getSessionId().equals(request.getSession().getId())){
	                 		logger.debug("compareing:"+session.getId()+"=="+csl.getSessionId()); 
//	                 		isExceed=true;
	                 	} 
                 } 
            	}
